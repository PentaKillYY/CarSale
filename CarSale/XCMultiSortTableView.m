...
//            }
//            
//            NSString *tagStr = [NSString stringWithFormat:@"%ld_%d", section, i];
//            subView.tag = (int)tagStr;
//            
//            NSString *columnStr = [NSString stringWithFormat:@"%ld_%d", section, i];
...
- (void)setUpTopHeaderScrollView {
    ...
    for (int i = 0; i < count; i++) {
        ...
        CGSize labelSize = [label calculateSize:[[datasource arrayDataForTopHeaderInTableView:self] objectAtIndex:i]];
        label.frame= CGRectMake((topHeaderW-labelSize.width)/2.0f, (topHeaderH-labelSize.height)/2.0f, labelSize.width, labelSize.height);
...
- (void)contentHeaderTap:(UITapGestureRecognizer *)recognizer {
    ...
    if (section == -1) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            [columnSortedTapFlags setObject:[NSNumber numberWithInt:(int)columnFlag] forKey:str];
...
- (void)singleHeaderClick:(NSIndexPath *)indexPath {
    ...
    [columnSortedTapFlags setObject:[NSNumber numberWithInt:(int)columnFlag] forKey:columnStr];
